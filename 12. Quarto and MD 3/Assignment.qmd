...
### Season `r params$seasons` summary

```{r}
episodes_no <- nrow(season_data)
avg_viewer <- mean(season_data$viewer)
```

Season 1 of Game of Thrones consisted of `r episodes_no` episodes that aired between April 17 and June 19, 2011 on HBO. The show gathered an average of `r avg_viewer` first-day TV viewers in the US, with the number growing from 2.22 to 3.04 million by the end of the season.
